...
//        String sql="SELECT img_imagen FROM \"imagenArticulo\" WHERE img_articulo = "+cod;
        ...
        try {
            ResultSet rs;
            PreparedStatement sentencia = conectar.getConexion().prepareStatement(sql);
...
//        String sql = "SELECT * FROM vendedor WHERE ven_codigo = " + cod + "";
    ...
    public static Vendedor busVendedorFactura(String cod) {
        ...
        try {
            HttpClient vendedor = new HttpClient(new OnHttpRequestComplete() {
                @Override
                    ...
                    if (status.isSuccess()) {
                        try {
                            if (status.getResult().equals("rows 0{}")) {
                                ...
                                dlgVentas.btnConfirmar.setEnabled(false);
                                dlgVentas.txtCodVendedor.requestFocus();
                                dlgVentas.txtCodVendedor.selectAll();
                            } else {
                                JSONObject jsonVendedor = new JSONObject(status.getResult());
                                for (int f = 0; /*f < jsonVendedor.length()*/; f++) {
                                    v.setNombreV(jsonVendedor.getJSONObject("" + f + "").get("NombreVendedor").toString());
                                }
                            }
            ...
            vendedor.excecute(urlServer() + "getbusVendedorFactura.php?param="+param);
        } catch (Exception e) {
