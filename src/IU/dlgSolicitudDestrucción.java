 ...
 * @author ADMIN
 */
public class dlgSolicitudDestrucción extends javax.swing.JDialog {
     ...
     * @param modal
     */
    public dlgSolicitudDestrucción(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        titulo();
        ...
        txtFechaFinal.setText(Fecha.formatoFecha(dcFecha.getText()));
        controlSalida.listarTLSDestruccion(tbDetalle, txtFechaFinal.getText().trim());
        Renders();
    ...
    private void nuevo() {
        dlgGestSolicitudDestrucción GR = new dlgGestSolicitudDestrucción(null, true);
    ...
    private void delSolicitud() {
        ...
        if (x < 0) {
            Mensajes.Sistema("Seleccione la Solicitud de Destrucción que desea anular");
        } else {
            int fila = dlgSolicitudDestrucción.tbDetalle.getSelectedRow();
            String estado = dlgSolicitudDestrucción.tbDetalle.getValueAt(fila, 8).toString();
            String estado2 = dlgSolicitudDestrucción.tbDetalle.getValueAt(fila, 9).toString();
            if (!estado.equals("PENDIENTE")) {
                Mensajes.Sistema("ANULACIÓN DE SOLICITUD RESTRINGIDA.\nMOTIVO: La Solicitud de Destrucción seleccionada ya se encuentra procesada.");
            } else {
                if (estado2.equals("ANULADO")) {
                    Mensajes.Sistema("NO ES POSIBLE ANULAR ESTA SOLICITUD.\nMOTIVO: La Solicitud de Destrucción seleccionada ya se encuentra anulada.");
                } else {
                    String msg;
                    int resp = JOptionPane.showConfirmDialog(this, "¿Seguro que desea Anular esta Solicitud de Destrucción?", "CONFIRMACIÓN DE ANULACIÓN", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
                    if (resp == JOptionPane.YES_OPTION) {
                        int id = Integer.parseInt(dlgSolicitudDestrucción.tbDetalle.getValueAt(fila, 0).toString());
                        msg = controlSalida.DelDestruccion(id);
                        if (msg.equals("OK")) {
                            CabecerasTablas.limpiarTablasTLS(tbDetalle);
                            CabecerasTablas.limpiarTablasRefTSNCP(tbREF);
                            CabecerasTablas.limpiarTablasDetalleRefTSNCP(tbDetalleS);
    ...
    private void Modificar() {
        int x = tbDetalle.getSelectedRow();
        if (x < 0) {
            Mensajes.Sistema("Seleccione la Solicitud de Destrucción que desea cambiar de estado");
        } else {
            String estado2 = dlgSolicitudDestrucción.tbDetalle.getValueAt(x, 9).toString();
            if (estado2.equals("ANULADO")) {
                ...
                try {
                    ...
                    dlgGestSolicitudDestrucción.txtCod.setText(tbDetalle.getValueAt(x, 0).toString());
                    dlgGestSolicitudDestrucción.txtCaja.setText(tbDetalle.getValueAt(x, 1).toString());
                    Object descripcion = tbDetalle.getValueAt(x, 3).toString();
                    dlgGestSolicitudDestrucción.cbProveedor.setSelectedItem(descripcion);
                    dlgGestSolicitudDestrucción.txtFecha.setText(Fecha.formatoFechaDd(tbDetalle.getValueAt(x, 4).toString()));
                    dlgGestSolicitudDestrucción.txtHora.setText(Fecha.FormatoHoraSinSSString(tbDetalle.getValueAt(x, 5).toString()));
                    if (tbDetalle.getValueAt(x, 8).toString().equals("PENDIENTE")) {
                        dlgGestSolicitudDestrucción.ckPendiente.setSelected(false);
                        dlgGestSolicitudDestrucción.ckPendiente.setEnabled(true);
                        dlgGestSolicitudDestrucción.ckTexto();
                    } else {
                        dlgGestSolicitudDestrucción.ckPendiente.setSelected(true);
                        dlgGestSolicitudDestrucción.ckPendiente.setEnabled(true);
                        dlgGestSolicitudDestrucción.ckTexto();
                    }
                    dlgGestSolicitudDestrucción.txtObs.setText(tbDetalle.getValueAt(x, 7).toString());
                    DecimalFormat df = new DecimalFormat("#,###");
                    dlgGestSolicitudDestrucción.txtTotal.setText(df.format(Integer.parseInt(tbDetalle.getValueAt(x, 6).toString().replace(".", "").replace(",", ""))));
                    dlgGestSolicitudDestrucción.btnNuevo.setEnabled(false);
                    controlSalida.listDetalleTLSDpModificacion(Integer.parseInt(dlgGestSolicitudDestrucción.txtCod.getText()), dlgGestSolicitudDestrucción.tbDetalle);
                    dlgGestSolicitudDestrucción.Renders();
                    dlgGestSolicitudDestrucción.opcion = "M";
                    GR.setVisible(true);
                } catch (Exception e) {
    ...
    private void initComponents() {
        ...
        jLabel1.setText("SOLICITUDES DE DESTRUCCIÓN DE LA FECHA");
        Blanco.add(jLabel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(465, 80, 410, 26));
    ...
    private void tbDetalleMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbDetalleMousePressed
        // TODO add your handling code here:
        CabecerasTablas.limpiarTablasDetalleRefTSNCP(tbDetalleS);
        controlSalida.listDetalleTLSD(tbDetalleS);
        CabecerasTablas.limpiarTablasRefTSNCP(tbREF);
        controlSalida.listRefTLSD(tbREF);
    ...
    private void btnModificarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnModificarActionPerformed
        ...
        if (tbDetalle.getSelectedRowCount() <= 0) {
            Mensajes.Sistema("Seleccione una Solicitud de Destrucción para Imprimir el Documento.");
        } else {
            prepararBD();
            int x = tbDetalle.getSelectedRow();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(dlgSolicitudDestrucción.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        }
        //</editor-fold>
        //</editor-fold>
        //</editor-fold>
        ...
        java.awt.EventQueue.invokeLater(() -> {
            dlgSolicitudDestrucción dialog = new dlgSolicitudDestrucción(new javax.swing.JFrame(), true);
