 ...
 * @author ADMIN
 */
public class dlgPagoCompra extends javax.swing.JDialog {
    ...
    public dlgPagoCompra(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        CabecerasTablas.DetallePagos(tbDetallePago);
        CargarCombos();
        txtIdPagos.setText(GestionarCompra.getCodigoPago());
        HabilitarDatos();
        lbPagar.setText(dlgCompras1.txtTotal.getText());
        Invisible();
    }
    ...
    public static void Renders() {
        dlgPagoCompra.tbDetallePago.getColumnModel().getColumn(8).setCellRenderer(new RenderDecimal1());
    }
    ...
    public static void CalcularDif() {
        ...
        dif = (pagado - pagar);
        dlgPagoCompra.lbDif.setText(df.format(dif));
    }
    ...
    private void initComponents() {
        ...
        jPanel2.add(txtIdPagos, new org.netbeans.lib.awtextra.AbsoluteConstraints(210, 20, 80, -1));
    ...
    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
        ...
        if (rbEfectivo.isSelected()) {
            if (txtMonto.getText().isEmpty()) {
                ...
                controlCompra1.addTablaPagoCompra(tbDetallePago);
    ...
    private void btnRestarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRestarActionPerformed
        // TODO add your handling code here:
        try {
            controlCompra1.delRenglonPagoCompra(tbDetallePago);
        } catch (Exception e) {
    ...
    private void btnGuardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGuardarActionPerformed
        ...
        if (tbDetallePago.getRowCount() == 0) {
            Mensajes.Sistema("Es necesario registrar el o los modos de pago");
        } else if (tbDetallePago.getRowCount() > 0) {
            if (Integer.parseInt(lbDif.getText().trim().replace(".", "").replace(",", "")) < 0) {
                Mensajes.error("El Pago detallado no es suficiente para liquidar el Total de la Factura Compra.\n\nVerifique:\n 1- El MONTO detallado.\n 2- Si existe más de una forma de pago.");
            } else {
                try {
                    ...
                    if (resp == JOptionPane.YES_OPTION) {
                        try {
                            ...
                            String sql = "insert into compra values(" + dlgCompras1.txtCod.getText() + "," + dlgCompras1.txtCaja.getText() + "," + dlgCompras1.txtCodProv.getText() + ", '" + dlgCompras1.lbCond.getText() + "','" + dlgCompras1.txtFactura.getText() + "','"
                                    + dlgCompras1.txtFecha.getText() + "','" + Fecha.darHora() + "'," + dlgCompras1.txtTotalL.getText() + ", 0,'ABONADO', " + dlgCompras1.txtExentaL.getText() + "," + dlgCompras1.txt5L.getText() + "," + dlgCompras1.txt10L.getText() + ",'S','" + usuario + "')";
                            stTransaccion.executeUpdate(sql);
                            int fila = dlgCompras1.tbDetalle.getRowCount();
                            for (int j = 0; j < fila; j++) {
                                String filas[] = {
                                    dlgCompras1.tbDetalle.getValueAt(j, 0).toString(), //0
                                    dlgCompras1.tbDetalle.getValueAt(j, 4).toString(), //1
                                    dlgCompras1.tbDetalle.getValueAt(j, 6).toString(), //2
                                    dlgCompras1.tbDetalle.getValueAt(j, 15).toString(), //3
                                    dlgCompras1.tbDetalle.getValueAt(j, 16).toString(), //4
                                    dlgCompras1.tbDetalle.getValueAt(j, 7).toString(), //5
                                    dlgCompras1.tbDetalle.getValueAt(j, 17).toString(),//6
                                    dlgCompras1.tbDetalle.getValueAt(j, 18).toString(),//7
                                    dlgCompras1.tbDetalle.getValueAt(j, 19).toString(),//8
                                    dlgCompras1.tbDetalle.getValueAt(j, 20).toString(),//9
                                    dlgCompras1.tbDetalle.getValueAt(j, 21).toString(),//10
                                };
                                sql = "insert into detalle_compra values(" + dlgCompras1.txtCod.getText() + "," + filas[0] + "," + filas[1] + "," + filas[2] + "," + filas[3] + "," + filas[5] + ")";
                            ...
                            sql = "INSERT INTO pagos_compras "
                                    + "VALUES(" + txtIdPagos.getText() + ", " + dlgCompras1.txtCaja.getText() + ", " + lbPagado.getText().replace(".", "").replace(",", "") + ", NOW(), '" + usuario + "', 'S')";
                            String sql2 = "INSERT INTO ref_pagos_compra "
                                                + "VALUES(" + txtIdPagos.getText() + ", " + dlgCompras1.txtCod.getText() + ")";
                            stTransaccion.executeUpdate(sql);
                            stTransaccion.executeUpdate(sql2);                          

                            int pagos = dlgPagoCompra.tbDetallePago.getRowCount();
                            for (int j = 0; j < pagos; j++) {
                                String filas[] = {
                                    dlgPagoCompra.tbDetallePago.getValueAt(j, 0).toString(), //idcompra     0
                                    dlgPagoCompra.tbDetallePago.getValueAt(j, 1).toString(), //idcaja       1
                                    dlgPagoCompra.tbDetallePago.getValueAt(j, 2).toString(), //pago con     2
                                    dlgPagoCompra.tbDetallePago.getValueAt(j, 4).toString(), //idbanco      3
                                    dlgPagoCompra.tbDetallePago.getValueAt(j, 5).toString(), //Nro cuenta   4
                                    dlgPagoCompra.tbDetallePago.getValueAt(j, 6).toString(), //Nro cheque   5
                                    dlgPagoCompra.tbDetallePago.getValueAt(j, 7).toString(), //fecha pago   6
                                    dlgPagoCompra.tbDetallePago.getValueAt(j, 8).toString(), //Monto        7
                                    dlgPagoCompra.tbDetallePago.getValueAt(j, 9).toString(), //RS           8
                                    dlgPagoCompra.tbDetallePago.getValueAt(j, 10).toString() //RUC          9                            
                                };
                                if (filas[2].equals("EFECTIVO")) {
                                    sql = "INSERT INTO detalle_pagos_compra "
                                            + "VALUES(" + txtIdPagos.getText() + ", '" + filas[2] + "', " + filas[7] + ")";
                                    stTransaccion.executeUpdate(sql);
                                } else if (filas[2].equals("CHEQUE")) {
                                    sql = "INSERT INTO detalle_pagos_compra "
                                            + "VALUES(" + txtIdPagos.getText() + ", '" + filas[2] + "', " + filas[7] + ")";
                                    String sql3 = "INSERT INTO cheque_compra (idpago, idcaja, idbanco, titular, ruc, nro_cuenta, nro_cheque, fecha_pago, monto, usuario, estado)"
                                            + "VALUES(" + txtIdPagos.getText() + "," + filas[1] + ",'" + filas[3] + "','" + filas[8] + "','" + filas[9] + "', '" + filas[4] + "', '" + filas[5] + "','" + dlgCompras1.txtFecha.getText() + "', " + filas[7] + ", '" + usuario + "', 'S')";
                                    stTransaccion.executeUpdate(sql);
                            ...
                            con.commit();
                            stTransaccion.close();
                            Mensajes.informacion("La Compra con factura N°:" + dlgCompras1.txtFactura.getText() + " \ny su detalle de Pago ha sido regitrado exitosamente.");
                            this.dispose();
                        } catch (SQLException e) {
                            try {
                                con.rollback();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(dlgPagoCompra.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                dlgPagoCompra dialog = new dlgPagoCompra(new javax.swing.JFrame(), true);
