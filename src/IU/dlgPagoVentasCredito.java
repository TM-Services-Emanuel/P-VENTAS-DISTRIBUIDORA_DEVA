...
import static IU.dlgRegistrarPagosFacturas.ActualizarTabla;
 ...
 * @author ADMIN
 */
public class dlgPagoVentasCredito extends javax.swing.JDialog {
    ...
    public dlgPagoVentasCredito(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        ...
        CargarCombos();
        HabilitarDatos();
        lbPagar.setText(dlgRegistrarPagosFacturas.txtDif.getText());
        CalcularDif();
        Invisible();
        txtIdPago.setText(GestionarCompra.getCodigoPagoCredito());
        dlgRegistrarPagosFacturas.nuevaRD();
    }
    ...
    public static void Renders() {
        dlgPagoVentasCredito.tbDetallePago.getColumnModel().getColumn(8).setCellRenderer(new RenderDecimal1());
    }
    ...
    public static void CalcularDif() {
        ...
        dif = (pagado - pagar);
        dlgPagoVentasCredito.lbDif.setText(df.format(dif));
    }
    ...
    private void initComponents() {
        ...
        jPanel2.add(lbIDBanco, new org.netbeans.lib.awtextra.AbsoluteConstraints(290, 20, 59, -1));
        ...
        jPanel2.add(btnAdd, new org.netbeans.lib.awtextra.AbsoluteConstraints(240, 20, 20, 20));
        ...
        jPanel2.add(btnRestar, new org.netbeans.lib.awtextra.AbsoluteConstraints(200, 20, 27, -1));
        jPanel2.add(txtIdPago, new org.netbeans.lib.awtextra.AbsoluteConstraints(360, 20, 70, -1));
        jPanel2.add(txtIdEmision, new org.netbeans.lib.awtextra.AbsoluteConstraints(140, 20, 50, -1));

        jLabel1.setFont(new java.awt.Font("Roboto", 1, 14)); // NOI18N
        jLabel1.setForeground(new java.awt.Color(255, 255, 255));
        jLabel1.setText("RECIBO NRO:");
        jPanel2.add(jLabel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(495, 25, 90, 23));

        txtRecibo.setEditable(false);
        txtRecibo.setBackground(new java.awt.Color(255, 255, 255));
        txtRecibo.setFont(new java.awt.Font("Roboto", 1, 14)); // NOI18N
        txtRecibo.setHorizontalAlignment(javax.swing.JTextField.CENTER);
        txtRecibo.setBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(0, 102, 102)));
        jPanel2.add(txtRecibo, new org.netbeans.lib.awtextra.AbsoluteConstraints(585, 25, 100, 23));
    ...
    lbPagar.setFont(new java.awt.Font("Roboto", 1, 16)); // NOI18N
    ...
    jPanel1.add(jLabel11, new org.netbeans.lib.awtextra.AbsoluteConstraints(260, 60, 120, 40));
    ...
    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
        ...
        if (rbEfectivo.isSelected()) {
            if (txtMonto.getText().isEmpty()) {
                ...
                controlCompra1.addTablaPagoVentasCredito(tbDetallePago);
    ...
    private void btnRestarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRestarActionPerformed
        // TODO add your handling code here:
        try {
            controlCompra1.delRenglonPagoVentaCreditos(tbDetallePago);
        } catch (Exception e) {
                ...
                try {
                    ...
                    if (resp == JOptionPane.YES_OPTION) {
                        try {
                            ...
                            String sql = "INSERT INTO pagos_ventas "
                                    + "VALUES(" + txtIdPago.getText() + ", " + dlgRegistrarPagosFacturas.txtCaja.getText() + "," + txtIdEmision.getText().trim() + ", '" + txtRecibo.getText() + "', " + lbPagado.getText().replace(".", "").replace(",", "") + ", NOW(), '" + usuario + "', 'S')";
                            String sql4 = "UPDATE puntoemision set facturaactual=" + Integer.parseInt(txtRecibo.getText().trim()) + " WHERE idemision=" + txtIdEmision.getText().trim();
                            String sql5 = "UPDATE ref set nventa=" + Integer.parseInt(txtRecibo.getText().trim()) + " WHERE idemision=" + txtIdEmision.getText().trim();
                            stTransaccion.executeUpdate(sql);
                            stTransaccion.executeUpdate(sql4);
                            stTransaccion.executeUpdate(sql5);

                            int pagos = dlgPagoVentasCredito.tbDetallePago.getRowCount();
                            for (int j = 0; j < pagos; j++) {
                                String filas[] = {
                                    dlgPagoVentasCredito.tbDetallePago.getValueAt(j, 0).toString(), //idventa      0
                                    dlgPagoVentasCredito.tbDetallePago.getValueAt(j, 1).toString(), //idcaja       1
                                    dlgPagoVentasCredito.tbDetallePago.getValueAt(j, 2).toString(), //pago con     2
                                    dlgPagoVentasCredito.tbDetallePago.getValueAt(j, 4).toString(), //idbanco      3
                                    dlgPagoVentasCredito.tbDetallePago.getValueAt(j, 5).toString(), //Nro cuenta   4
                                    dlgPagoVentasCredito.tbDetallePago.getValueAt(j, 6).toString(), //Nro cheque   5
                                    dlgPagoVentasCredito.tbDetallePago.getValueAt(j, 7).toString(), //fecha pago   6
                                    dlgPagoVentasCredito.tbDetallePago.getValueAt(j, 8).toString(), //Monto        7
                                    dlgPagoVentasCredito.tbDetallePago.getValueAt(j, 9).toString(), //RS           8
                                    dlgPagoVentasCredito.tbDetallePago.getValueAt(j, 10).toString() //RUC          9                            
                                };
                                if (filas[2].equals("EFECTIVO")) {
                                    sql = "INSERT INTO detalle_pagos_venta "
                                            + "VALUES(" + txtIdPago.getText() + ", '" + filas[2] + "', " + filas[7] + ")";
                                    stTransaccion.executeUpdate(sql);
                                } else if (filas[2].equals("CHEQUE")) {
                                    sql = "INSERT INTO detalle_pagos_venta "
                                            + "VALUES(" + txtIdPago.getText() + ", '" + filas[2] + "', " + filas[7] + ")";
                                    String sql2 = "INSERT INTO cheque_venta (idpago, idcaja, idbanco, titular, ruc, nro_cuenta, nro_cheque, fecha_pago, monto, usuario, estado)"
                            ...
                            int nc = dlgRegistrarPagosFacturas.tbDetalleNC.getRowCount();
                            for (int j = 0; j < nc; j++) {
                                String filas[] = {
                                    dlgRegistrarPagosFacturas.tbDetalleNC.getValueAt(j, 0).toString(), //idnc      0
                                };
                                sql = "INSERT INTO ref_pagos_nc_venta "
                            ...
                            if (dlgRegistrarPagosFacturas.Seleccionados(11)) {
                                for (int i = 0; i < dlgRegistrarPagosFacturas.tbFacturasCredito.getRowCount(); i++) {
                                    ...
                                    if (sel) {
                                        String filas[] = {
                                            dlgRegistrarPagosFacturas.tbFacturasCredito.getModel().getValueAt(i, 0).toString()//0 //idventa
                                        };
                                        sql = "INSERT INTO ref_pagos_venta "
                                                + "VALUES(" + txtIdPago.getText() + ", " + filas[0] + ")";
                                        String sql2 = "UPDATE factura SET estado='ABONADO' WHERE fac_codigo=" + filas[0];
                                        String sql3 = "UPDATE nc_cliente SET liquidado='S' WHERE idfactura=" + filas[0];
                                        stTransaccion.executeUpdate(sql);
                                        stTransaccion.executeUpdate(sql2);
                            ...
                            String Letra = d.Convertir((lbPagar.getText().replace(".", "").replace(",", "")), true);
                            ImprimirRecibo(Integer.parseInt(txtIdPago.getText()), Letra );
                            this.dispose();
                        } catch (SQLException e) {
                            try {
                                con.rollback();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(dlgPagoVentasCredito.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                dlgPagoVentasCredito dialog = new dlgPagoVentasCredito(new javax.swing.JFrame(), true);
