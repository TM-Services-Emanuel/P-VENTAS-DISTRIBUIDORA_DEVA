...
public class dlgSeleccionarSalidasNC extends javax.swing.JDialog {

    CabecerasTablas cabe = new CabecerasTablas();

    public dlgSeleccionarSalidasNC(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        titulo();
        CabecerasTablas.consNCpSolicitud(tbSalidas);
        CabecerasTablas.limpiarTablasSalidasNCP(tbSalidas);
        controlSalida.listarSalidasNCP(tbSalidas, Integer.parseInt(dlgGestSolicitudNCP.txtcodProv.getText().trim()));
        Renders();
    ...
    private void btnActualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnActualizarActionPerformed
        // TODO add your handling code here:
        try {
            dlgGestSolicitudNCP.txtObs.setText("");
            CabecerasTablas.limpiarTablasSalidasDNCP(dlgGestSolicitudNCP.tbDetalle);
            CabecerasTablas.limpiarTablasRefSolNCP(dlgGestSolicitudNCP.tbSalidas);
            if (Seleccionados(7)) {
                for (int i = 0; i < tbSalidas.getRowCount(); i++) {
                    boolean sel = (boolean) tbSalidas.getValueAt(i, 7);
                    if (sel) {
                        dlgGestSolicitudNCP.txtObs.append("OPERACIÓN SALIDA NRO:" + tbSalidas.getValueAt(i, 0) + "\n");
                        int x = Integer.parseInt(tbSalidas.getValueAt(i, 0).toString());
                        controlSalida.addTablaSolicitudNCP(dlgGestSolicitudNCP.tbDetalle, x);
                        insertar(i, x, dlgGestSolicitudNCP.tbSalidas);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(dlgSeleccionarSalidasNC.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                dlgSeleccionarSalidasNC dialog = new dlgSeleccionarSalidasNC(new javax.swing.JFrame(), true);
